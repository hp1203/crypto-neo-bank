    ...
    <AppLayout>
      ...
      <Grid cols={3}>
            ...
            <Card>
                ...
                <div id="jh-stats-positive" className="flex flex-col justify-center items-center">
                        ...
                        <p className="text-3xl font-semibold text-center text-gray-800">{paymentLinks.length}</p>
                        <p className="text-lg text-center text-gray-500">Payment Links</p>
        ...
        <Grid cols={2}>
              ...
              <TableBody className="overflow-x-scroll">
                    {
                      payments.length > 0 ?
                        payments.map((payment, index) => (
                            ...
                            <TableCol>{payment.amount ? utils.formatEther(payment.amount.toString()) : 0.0} {payment.currency}</TableCol>
                            <TableCol>{payment.fee}</TableCol>
                            <TableCol>{moment.unix(payment.timestamp).toString()}</TableCol>
                            </TableRow>
                            </>
                        )) : (
                          <TableRow className="w-full">
                          <TableCol colSpan="8" className="text-center border">
                            <p className="text-base text-gray-500">No Payments Found</p>
                          </TableCol>
                          </TableRow>
                        )
                    }
          ...
          <Card>
            ...
            paymentLinks.map((link, index) => (
                <ListItem title={link.url} subTitle={link.type} key={index} left={(
                  <div className="flex flex-col items-end">
                    <a className="text-xs text-green-600" target="_blank" href={`pay/${link.url}`}><FiExternalLink className="text-base text-violet-500 cursor-pointer"/></a>
                  </div>
                )}/>
              ))
            }
